...
const sendShowReminders = inngest.createFunction(
  ...
  { cron: "0 */8 * * *", timezone: "Asia/Kolkata" }, // every 8 hours IST
  async ({ step }) => {
    const now = new Date();
    const in8Hours = new Date(now.getTime() + 8 * 60 * 60 * 1000);
    const windowStart = new Date(in8Hours.getTime() - 10 * 60 * 1000);

    // 1. Fetch shows happening in the next 8 hours
    const reminderTasks = await step.run("prepare-reminder-tasks", async () => {
      try {
        ...
        for (const show of shows) {
          ...
          // occupied seats map: {A1: userId, B2: userId}
          const userIds = [
          ...
          for (const user of users) {
            if (!user.email) continue; // skip invalid entries
        ...
        console.error("Error preparing reminder tasks:", err);
        return [];
      }
    });

    // No reminders → return safely
            ...
            await sendEmail({
              ...
              body: `
                <div style="font-family: Arial, sans-serif; padding: 20px;">
                  ...
                  <p>It begins in approximately <strong>8 hours</strong>. Don’t miss it!</p>
                  <br/>
